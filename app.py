...
def call_gemini(history, user_text, image_pil=None):
    ...
    for turn in history[-8:]:
        ...
        if built: contents.append(types.Content(role=role,parts=built))
    new_parts=[]
    if image_pil:
        new_parts.append(types.Part(inline_data=types.Blob(mime_type="image/jpeg",data=pil_to_bytes(image_pil,75))))
    new_parts.append(types.Part.from_text(text=user_text or "Describe this image."))
    ...
    t=threading.Thread(target=_call,daemon=True); t.start(); t.join(timeout=GEMINI_TIMEOUT)
    if t.is_alive(): raise TimeoutError("Gemini timed out")
...
def view_shared(share_id):
    ...
    if not chat: return "<h2>Chat not found or expired.</h2>",404
