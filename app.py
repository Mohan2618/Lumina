...
try:
    if GEMINI_API_KEY:
        ...
        print("⚠️  No GEMINI_API_KEY — running in free/fallback mode")
...
2. Perform medical image analysis — detect anomalies, describe findings, suggest specialist types
...
def resize_for_api(img, max_dim=1024):
    w, h = img.size
    if max(w, h) > max_dim:
...
def call_gemini(history: list, user_text: str, image_pil=None) -> str:
    if not gemini_client:
        return free_reply(user_text, image_pil is not None or bool(history), image_pil)
    ...
    try:
        ...
        for turn in history:
            ...
            for p in parts_raw:
                ...
                elif isinstance(p, dict) and p.get("mime_type") and p.get("data") != '[stripped]':
                    try:
                        raw = p.get("data", "")
        ...
        new_parts = []
        if image_pil:
            img_api = resize_for_api(image_pil)
            new_parts.append(gtypes.Part(
                inline_data=gtypes.Blob(mime_type="image/jpeg", data=pil_to_bytes(img_api, 80))
            ))
...
def analyze_image_basic(img):
    ...
    return dict(w=w, h=h, mr=mr, mg=mg, mb=mb, brightness=brightness,
                dom=dom, bright=bright, orient=orient, detail=detail, contrast=contrast)
...
def free_reply(prompt, has_image, img=None):
    ...
    replies = {
        ...
        r'what can you do|help|feature': (
            ...
            "🔬 **Detection:** Face detect, edge detection, color palette\n"
...
def detect_op(p):
    ...
    if re.search(r'\bblur\b|\bsmooth\b|\bsoft\b', p) and not re.search(r'deblur|un.?blur|fix.?blur', p):
    ...
    if re.search(r'\blomo\b|\bvintage filter\b', p):
        return "Applying lomo/vintage effect!\n<OP>{\"intent\":\"lomo\",\"params\":{}}</OP>"
    # COLOR
    ...
    if re.search(r'\becg\b|\belectrocardiogram\b', p):
    ...
    if re.search(r'\bvintage\b', p):
        return "Applying vintage tone!\n<OP>{\"intent\":\"sepia\",\"params\":{}}</OP>"
    # GENERATE — must check last to avoid false positives
    if re.search(r'\bgenerate\b|\bcreate.?image\b|\bmake.?image\b|\bgenerate.?image\b', p):
        # Extract the subject/prompt
        cleaned = re.sub(r'\b(generate|create|make|an?|the|image|picture|photo|of|me|a|please|can you)\b', '', p, flags=re.IGNORECASE).strip()
        if not cleaned or len(cleaned) < 3:
            cleaned = p
        return f"Generating image for you!\n<OP>{{\"intent\":\"generate_image\",\"params\":{{\"prompt\":\"{cleaned}\"}}}}</OP>"
...
def generate_image_from_prompt(prompt: str):
    """Generate image using Gemini image generation or create a rich placeholder"""
    if gemini_client:
        ...
        for model_name in models_to_try:
            try:
                if "imagen" in model_name:
                    ...
                    response = gemini_client.models.generate_content(
                        model=model_name,
                        contents=f"Generate a high quality, detailed, photorealistic image of: {prompt}",
                        config=gtypes.GenerateContentConfig(
                ...
                print(f"Image gen model {model_name} failed: {e}")
                continue

    return _create_placeholder_image(prompt)

def _create_placeholder_image(prompt: str):
    """Create a beautiful placeholder when generation is unavailable"""
    w, h = 512, 512
    ...
    # Draw text panel
    draw.rectangle([30, h // 2 - 80, w - 30, h // 2 + 90], fill=(0, 0, 0, 160))
    text = (prompt[:50] + "...") if len(prompt) > 50 else prompt
    draw.text((w // 2, h // 2 - 55), "🎨 Image Generation", fill=(255, 255, 255), anchor="mm")
    draw.text((w // 2, h // 2 - 20), f'"{text}"', fill=(200, 240, 220), anchor="mm")
    draw.text((w // 2, h // 2 + 20), "Add GEMINI_API_KEY to", fill=(180, 210, 200), anchor="mm")
    draw.text((w // 2, h // 2 + 50), "enable real AI generation", fill=(180, 210, 200), anchor="mm")
    return img


# ─────────────────────────────────────────────────────────────
#  CORE IMAGE PROCESSOR — all operations with robust error handling
# ─────────────────────────────────────────────────────────────
def process_image(img, intent, params):
    ...
    try:
        ...
        if intent == 'resize':
            return img.resize((max(1, int(params.get('width', 512))), max(1, int(params.get('height', 512)))), Image.LANCZOS)

        if intent == 'resize_pct':
            p = float(params.get('pct', 50)) / 100.0
        ...
        if intent == 'sharpen':
            result = img.filter(ImageFilter.UnsharpMask(radius=2, percent=200, threshold=3))
            return result.filter(ImageFilter.UnsharpMask(radius=1, percent=100, threshold=2))

        if intent == 'edge':
            gray = ImageOps.grayscale(img)
            edges = gray.filter(ImageFilter.FIND_EDGES)
            arr = np.clip(np.array(edges) * 4, 0, 255).astype(np.uint8)
        ...
        if intent == 'sketch':
            ...
            arr_b = np.array(inv_blur, dtype=np.float32) + 1.0
            result = np.clip(arr_g * 255.0 / arr_b, 0, 255).astype(np.uint8)
            return ImageEnhance.Contrast(Image.fromarray(result)).enhance(1.5).convert("RGB")

        if intent == 'cartoon':
            cv_img = pil_to_cv2(img)
            # Bilateral filter for paint effect
            color = cv_img.copy()
            for _ in range(6):
        ...
        if intent == 'watercolor':
            cv_img = pil_to_cv2(img)
            # Multiple bilateral passes
            water = cv_img.copy()
            for _ in range(3):
                water = cv2.bilateralFilter(water, 9, 75, 75)
            # Slight edge enhancement
            gray = cv2.cvtColor(cv_img, cv2.COLOR_BGR2GRAY)
            edges = cv2.Laplacian(gray, cv2.CV_8U, ksize=5)
            edges_inv = cv2.bitwise_not(edges)
            edges_3ch = cv2.cvtColor(edges_inv, cv2.COLOR_GRAY2BGR)
            result = cv2.addWeighted(water, 0.9, edges_3ch, 0.1, 0)
            return ImageEnhance.Color(cv2_to_pil(result)).enhance(1.3)
        ...
        if intent == 'neon_glow':
            ...
            edges_blurred = cv2.GaussianBlur(edges, (0, 0), 5)
            # Dark background
            dark = (arr.astype(np.float32) * 0.12).astype(np.uint8)
            # Multi-color neon
            neon = np.zeros_like(arr, dtype=np.float32)
            neon[:, :, 0] = edges_blurred.astype(np.float32) * 0.3   # R
            neon[:, :, 1] = edges_blurred.astype(np.float32)          # G
            neon[:, :, 2] = edges_blurred.astype(np.float32)          # B
            glow1 = cv2.GaussianBlur(neon.astype(np.uint8), (21, 21), 0).astype(np.float32)
            glow2 = cv2.GaussianBlur(neon.astype(np.uint8), (51, 51), 0).astype(np.float32)
            result = np.clip(dark.astype(np.float32) + glow1 * 3 + glow2 * 1.5, 0, 255).astype(np.uint8)
        ...
        if intent == 'glitch':
            ...
            for _ in range(25):
                y = rng.randint(0, h - 5)
                h2 = rng.randint(1, max(2, h // 20))
                shift = rng.randint(-50, 50)
                arr[y:y + h2] = np.roll(arr[y:y + h2], shift, axis=1)
            arr[:, :, 0] = np.roll(arr[:, :, 0], 10, axis=1)
            arr[:, :, 2] = np.roll(arr[:, :, 2], -10, axis=1)
            return Image.fromarray(arr)

        if intent == 'halftone':
            gray = np.array(ImageOps.grayscale(img))
            h2, w2 = gray.shape
            dot_size = max(4, min(12, min(h2, w2) // 60))
        ...
        if intent == 'lomo':
            ...
            vig = 1 - np.clip(dist / (max(h2, w2) * 0.65), 0, 1) ** 2 * 0.75
            arr = arr * vig[:, :, np.newaxis]
            return ImageEnhance.Contrast(Image.fromarray(np.clip(arr, 0, 255).astype(np.uint8))).enhance(1.3)

        if intent == 'cross_process':
            arr = np.array(img).astype(np.uint8)
            lut_r = np.array([min(255, int(i ** 1.15)) if i < 128 else min(255, int(255 - (255 - i) ** 1.05)) for i in range(256)], dtype=np.uint8)
            ...
            result = arr.copy()
            result[:, :, 0] = lut_r[arr[:, :, 0]]
            result[:, :, 1] = lut_g[arr[:, :, 1]]
            result[:, :, 2] = lut_b[arr[:, :, 2]]
        ...
        if intent == 'pop_art':
            w2, h2 = img.size
            half_w, half_h = max(1, w2 // 2), max(1, h2 // 2)
            ...
            for i, c in enumerate(colors):
                ...
                gray_s = np.array(ImageOps.grayscale(small))
                _, thresh = cv2.threshold(gray_s, 120, 255, cv2.THRESH_BINARY)
        ...
        if intent == 'pointillism':
            ...
            n_dots = min(60000, h2 * w2 // 4)
            rng = np.random.RandomState(0)
            ys = rng.randint(0, h2, size=n_dots)
            xs = rng.randint(0, w2, size=n_dots)
            for y, x in zip(ys, xs):
                color = tuple(int(c) for c in arr[y, x])
                cv2.circle(canvas, (int(x), int(y)), dot_size, color, -1)
        ...
        if intent == 'ascii_art':
            ...
            for r in range(rows):
                for c in range(cols):
                    ...
                    avg = patch.mean() if patch.size > 0 else 128
                    char_idx = int(avg / 255.0 * (len(chars) - 1))
                    ch = chars[char_idx]
                    brightness = int(avg)
                    color = (brightness, min(255, int(brightness * 1.1)), min(255, int(brightness * 0.9)))
                    draw.text((c * cell, r * cell), ch, fill=color)
            return canvas
        ...
        if intent == 'tilt_shift':
            arr = np.array(img)
            h2, w2 = arr.shape[:2]
            blur_arr = cv2.GaussianBlur(arr, (0, 0), 20)
            ...
            for y in range(h2):
                ...
                if dist < zone:
                    ...
                    mask[y] = 1.0 - (dist - zone) / (zone * 2.0)
            mask3 = mask[:, :, np.newaxis]
            result = arr * mask3 + blur_arr * (1 - mask3)
            return ImageEnhance.Color(Image.fromarray(np.clip(result, 0, 255).astype(np.uint8))).enhance(1.5)

        if intent == 'bokeh':
            arr = np.array(img)
            h2, w2 = arr.shape[:2]
            blur_arr = cv2.GaussianBlur(arr, (0, 0), 25)
            cy, cx = h2 // 2, w2 // 2
            rr = min(h2, w2) // 3
            Y, X = np.ogrid[:h2, :w2]
            dist = np.sqrt((X - cx) ** 2 + (Y - cy) ** 2)
            mask = np.clip(1 - (dist - rr * 0.7) / (rr * 0.8), 0, 1)[:, :, np.newaxis]
            result = arr * mask + blur_arr * (1 - mask)
            return Image.fromarray(np.clip(result, 0, 255).astype(np.uint8))
        ...
        if intent == 'noise':
            ...
            grain = np.random.normal(0, 25, arr.shape)
        ...
        if intent == 'xray_enhance':
            ...
            result = cv2.LUT(np.clip(sharpened, 0, 255).astype(np.uint8), table)
            return Image.fromarray(result).convert("RGB")

        if intent == 'mri_enhance':
            gray = np.array(ImageOps.grayscale(img))
            p2, p98 = np.percentile(gray, 2), np.percentile(gray, 98)
            normalized = np.clip((gray.astype(np.float32) - p2) / max(p98 - p2, 1) * 255, 0, 255).astype(np.uint8)
        ...
        if intent == 'ct_enhance':
            ...
            windowed = np.clip((gray.astype(np.float32) - p5) / max(p95 - p5, 1) * 255, 0, 255).astype(np.uint8)
            cl = cv2.createCLAHE(clipLimit=3.5, tileGridSize=(8, 8))
            result = cl.apply(windowed)
            sharpened = cv2.filter2D(result, -1, np.array([[0, -0.5, 0], [-0.5, 3, -0.5], [0, -0.5, 0]]))
        ...
        if intent == 'face_detect':
            ...
            for (x, y, fw, fh) in faces:
                cv2.rectangle(cv_img, (x, y), (x + fw, y + fh), (65, 225, 174), 3)
                cv2.putText(cv_img, f"Face {fw}x{fh}", (x, y - 10),
        ...
        if intent == 'color_palette':
            ...
            _, labels, centers = cv2.kmeans(Z, K, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)
            pal_h = 80
            palette = np.zeros((pal_h, w2, 3), dtype=np.uint8)
        ...
        if intent == 'quality_check':
            ...
            for i, (text, color) in enumerate(texts):
                cv2.putText(result, text, (12, 35 + i * 28), cv2.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
            return cv2_to_pil(result)
        ...
        if intent == 'super_resolution':
            ...
            result = upscaled.filter(ImageFilter.UnsharpMask(radius=1.5, percent=180, threshold=3))
            return result.filter(ImageFilter.UnsharpMask(radius=0.7, percent=100, threshold=2))
        ...
        print(f"process_image error [{intent}]: {e}\n{traceback.format_exc()}")
        return None
    ...
    try:
        ...
        if file and file.filename:
            try:
                ...
                return jsonify({"message": f"⚠️ Could not read uploaded image: {e}"}), 200

        # Extract last image from history (for context-aware ops)
        last_image_pil = None
        if not image_pil:
            for turn in reversed(history):
                for p in turn.get("parts", []):
                    if isinstance(p, dict) and p.get("mime_type", "").startswith("image/") \
                       and p.get("data") and p.get("data") != '[stripped]':
                        try:
                            raw = p["data"]
                            if isinstance(raw, str):
                                raw = base64.b64decode(raw)
        ...
        if intent == 'info' and active_image:
            ...
            # Image generation doesn't need an existing image
            dummy = active_image or Image.new("RGB", (2, 2))
            result_img = process_image(dummy, intent, params or {})
            if result_img:
                result_b64 = pil_to_base64(result_img)
            else:
                clean_reply += "\n\n⚠️ Image generation failed. Please add a GEMINI_API_KEY."
            ...
            if result_img:
                ...
                clean_reply += "\n\n⚠️ Operation failed. Please try again."
        ...
        if image_pil:
            try:
                new_user_parts.append({
                    ...
                    "data": base64.b64encode(pil_to_bytes(image_pil, 70)).decode()
                })
            except Exception:
                pass
