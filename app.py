...
GEMINI_MODEL   = "gemini-2.0-flash"   # free tier, supports images
...
def call_gemini(history: list, user_text: str, image_pil=None) -> str:
    """
    history: list of {"role": "user"/"model", "parts": [str or dict]} dicts
    Sends the full conversation to Gemini and returns the reply text.
    """
    ...
    for turn in history:
        ...
        for p in parts:
            if isinstance(p, str):
                ...
                if p.get("mime_type"):
                    # image stored as base64 string in history
                    raw = p.get("data", "")
...
def process_image(img, intent, params):
    ...
    if intent == 'clahe':
        cv_img=pil_to_cv2(img); lab=cv2.cvtColor(cv_img,cv2.COLOR_BGR2LAB)
        l,a,b=cv2.split(lab); cl=cv2.createCLAHE(clipLimit=3.0,tileGridSize=(8,8))
    ...
    if intent == 'xray_enhance':
        ...
        sharpened=cv2.filter2D(cl.apply(gray),-1,np.array([[-1,-1,-1],[-1,9,-1],[-1,-1,-1]]))
...
def process():
    try:
        ...
        if "API_KEY_INVALID" in err or "API key not valid" in err:
            msg = "⚠️ Invalid Gemini API key. Check your GEMINI_API_KEY secret in HF Settings."
        elif "quota" in err.lower() or "429" in err:
            msg = "⚠️ Free API limit reached. Wait 1 minute and try again (free: 15 req/min)."
        elif "not found" in err.lower() or "404" in err:
            msg = "⚠️ Model not available. Please check your Gemini API key region."
        else:
            msg = f"⚠️ Error: {err}"
