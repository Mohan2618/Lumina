...
print(f"[INIT] GEMINI key present: {bool(GEMINI_API_KEY)}")
...
5. Generate images from text descriptions (like Grok, Gemini, ChatGPT)
6. Remember the full conversation context including previously uploaded images

IMPORTANT: You MUST respond to ALL user messages, including general questions, greetings, and topics unrelated to images.
...
- ALWAYS add disclaimer: "⚠️ This is AI analysis, not a medical diagnosis. Please consult a qualified medical professional."
...
def call_claude(history: list, user_text: str, image_pil=None) -> str:
    ...
    for turn in history[-10:]:
        ...
        for p in parts:
            ...
            elif isinstance(p, dict) and p.get("mime_type"): content += "[image] "
        if content.strip():
...
def call_gemini_fast(history: list, user_text: str, image_pil=None) -> str:
    ...
    new_parts = []
    if image_pil:
        new_parts.append(types.Part(inline_data=types.Blob(mime_type="image/jpeg", data=pil_to_bytes(image_pil, quality=75))))
    new_parts.append(types.Part.from_text(text=user_text if user_text else "Please describe and analyze this image in detail."))
    ...
    def _call():
        try:
            response = gemini_client.models.generate_content(
                ...
                config=types.GenerateContentConfig(system_instruction=SYSTEM_PROMPT, max_output_tokens=1024, temperature=0.7)
            )
            result[0] = response.text
        except Exception as e:
            error[0] = e
    t = threading.Thread(target=_call, daemon=True)
    t.start(); t.join(timeout=GEMINI_TIMEOUT)
    if t.is_alive(): raise TimeoutError(f"Gemini timed out after {GEMINI_TIMEOUT}s")
...
def generate_image_from_prompt(prompt: str):
    print(f"[Flux] Generating image for: {prompt[:100]}...")
    hf_token = os.environ.get("HF_TOKEN")
    if not hf_token:
        return create_flux_placeholder(prompt, "Add HF_TOKEN secret to enable image generation")
    try:
        ...
        print(f"[Flux] Failed: {str(e)[:150]}")
        return create_flux_placeholder(prompt, "Image generation busy. Try again shortly.")


def create_flux_placeholder(prompt: str, status="Generating..."):
    w, h = 1024, 1024
...
def free_reply(prompt, has_image, img=None):
    ...
    if re.search(r'who (created|made|built|developed|designed) you|who are your (creator|developer|maker)|your creator|your developer', p):
        return ("I'm **Lumina**, created by a talented team of four developers:\n\n"
                "1. **Mohan Lingabathina**\n2. **Hevendra Bage**\n3. **Sowrya**\n4. **Karthikeya**\n\n"
                "We were built as a project on HuggingFace Spaces. How can I help you today? 😊")
    if re.search(r'who are you|what are you|introduce yourself', p):
        return ("I'm **Lumina** — an advanced AI image processing assistant created by Mohan Lingabathina, "
                "Hevendra Bage, Sowrya, and Karthikeya! I can analyze images, apply creative filters, "
                "do medical imaging, generate art, and much more. Upload an image or ask me anything!")
    ...
    if op: return op
    replies={
        r'hello|hi\b|hey\b': "Hi there! 👋 I'm **Lumina**, your AI image assistant. What can I help you with?",
    ...
    if p:
        return (f"I understand you're asking about: *\"{prompt[:80]}\"*\n\n"
                "I'm Lumina, your AI image assistant. Connect the GEMINI_API_KEY for full conversational answers!\n\n"
                "**I can help with:** 🖼️ Image analysis & filters | 🤖 Image generation | 🏥 Medical analysis")
    return "Upload an image and ask me to describe it, apply a filter, or generate a new image!"
...
def detect_op(p):
    ...
    if re.search(r'\bgenerat\w*\b', p):
        ...
        if not prompt_match or len(prompt_match) < 5: prompt_match = p
    ...
    if re.search(r'\bsepia\b',p): return "Applying sepia tone!\n<OP>{\"intent\":\"sepia\",\"params\":{}}</OP>"
    if re.search(r'\bvintage filter\b|\blomo\b',p): return "Applying vintage/lomo effect!\n<OP>{\"intent\":\"lomo\",\"params\":{}}</OP>"
    if re.search(r'\bneon\b|\bglow\b',p): return "Applying neon glow effect!\n<OP>{\"intent\":\"neon_glow\",\"params\":{}}</OP>"
    if re.search(r'\bglitch\b',p): return "Applying glitch art effect!\n<OP>{\"intent\":\"glitch\",\"params\":{}}</OP>"
    if re.search(r'\bhalftone\b|\bdot pattern\b',p): return "Applying halftone effect!\n<OP>{\"intent\":\"halftone\",\"params\":{}}</OP>"
    ...
    if re.search(r'\bcartoon\b|\bcomic\b|\banime\b',p): return "Applying cartoon effect!\n<OP>{\"intent\":\"cartoon\",\"params\":{}}</OP>"
    if re.search(r'\bwatercolor\b',p): return "Applying watercolor effect!\n<OP>{\"intent\":\"watercolor\",\"params\":{}}</OP>"
    if re.search(r'\boil paint\b|\boil art\b',p): return "Applying oil painting effect!\n<OP>{\"intent\":\"oil_painting\",\"params\":{}}</OP>"
    if re.search(r'\bpop art\b|\bwarhol\b',p): return "Applying pop art effect!\n<OP>{\"intent\":\"pop_art\",\"params\":{}}</OP>"
    if re.search(r'\bstained glass\b',p): return "Applying stained glass effect!\n<OP>{\"intent\":\"stained_glass\",\"params\":{}}</OP>"
    if re.search(r'\bpointillis\b|\bdot art\b',p): return "Applying pointillism effect!\n<OP>{\"intent\":\"pointillism\",\"params\":{}}</OP>"
    if re.search(r'\bascii\b',p): return "Converting to ASCII art!\n<OP>{\"intent\":\"ascii_art\",\"params\":{}}</OP>"
    if re.search(r'\bthermal\b|\bheat map\b|\binfrared\b',p): return "Applying thermal vision effect!\n<OP>{\"intent\":\"thermal_vision\",\"params\":{}}</OP>"
    if re.search(r'\bemboss\b',p): return "Embossing!\n<OP>{\"intent\":\"emboss\",\"params\":{}}</OP>"
    if re.search(r'\bdouble exposure\b|\bdouble.?exp\b',p): return "Applying double exposure effect!\n<OP>{\"intent\":\"double_exposure\",\"params\":{}}</OP>"
    if re.search(r'\btilt.?shift\b|\bminiature\b',p): return "Applying tilt-shift/miniature effect!\n<OP>{\"intent\":\"tilt_shift\",\"params\":{}}</OP>"
    if re.search(r'\bbokeh\b|\bdepth of field\b|\bblur background\b',p): return "Applying bokeh/depth-of-field effect!\n<OP>{\"intent\":\"bokeh\",\"params\":{}}</OP>"
    if re.search(r'\bhdr\b|\bhigh dynamic range\b',p): return "Applying HDR effect!\n<OP>{\"intent\":\"hdr\",\"params\":{}}</OP>"
    if re.search(r'\bfisheye\b|\bfish.?eye\b',p): return "Applying fisheye lens effect!\n<OP>{\"intent\":\"fisheye\",\"params\":{}}</OP>"
    if re.search(r'\bcross process\b|\bcolor shift\b',p): return "Applying cross process effect!\n<OP>{\"intent\":\"cross_process\",\"params\":{}}</OP>"
    if re.search(r'\bduotone\b',p): return "Applying duotone effect!\n<OP>{\"intent\":\"duotone\",\"params\":{}}</OP>"
    if re.search(r'\bmosaic\b',p): return "Applying mosaic effect!\n<OP>{\"intent\":\"mosaic\",\"params\":{}}</OP>"
    ...
    if re.search(r'\bshadow\b|\bhighlight\b',p): return "Adjusting shadows & highlights!\n<OP>{\"intent\":\"shadows_highlights\",\"params\":{}}</OP>"
    if re.search(r'\bclahe\b|\bequali\b',p): return "Enhancing with CLAHE!\n<OP>{\"intent\":\"clahe\",\"params\":{}}</OP>"
    if re.search(r'\bdenois\b|\bclean\b|\bnoise.?remov\b',p): return "Denoising!\n<OP>{\"intent\":\"denoise\",\"params\":{}}</OP>"
    if re.search(r'\bxray\b|x-ray|x ray|x.?ray',p): return "X-ray enhancement!\n<OP>{\"intent\":\"xray_enhance\",\"params\":{}}</OP>"
    if re.search(r'\bmri\b',p): return "MRI enhancement!\n<OP>{\"intent\":\"mri_enhance\",\"params\":{}}</OP>"
    if re.search(r'\bct scan\b|\bct.?enhance\b',p): return "CT scan enhancement!\n<OP>{\"intent\":\"ct_enhance\",\"params\":{}}</OP>"
    if re.search(r'\bfundus\b|\bretina\b|\beye scan\b',p): return "Fundus/retinal enhancement!\n<OP>{\"intent\":\"fundus_enhance\",\"params\":{}}</OP>"
    if re.search(r'\bskin\b|\bderma\b|\blesion\b',p): return "Analyzing skin!\n<OP>{\"intent\":\"skin_analyze\",\"params\":{}}</OP>"
    if re.search(r'\bwound\b|\binjur\b',p): return "Analyzing wound!\n<OP>{\"intent\":\"wound_analyze\",\"params\":{}}</OP>"
    if re.search(r'\bsegment\b|\botsu\b|\bthreshold\b|\bbinary\b',p): return "Segmenting!\n<OP>{\"intent\":\"segment\",\"params\":{}}</OP>"
    if re.search(r'\bsobel\b|\bgradient\b',p): return "Sobel gradient!\n<OP>{\"intent\":\"sobel\",\"params\":{}}</OP>"
    if re.search(r'\bcanny\b',p): return "Canny edge detection!\n<OP>{\"intent\":\"canny\",\"params\":{}}</OP>"
    if re.search(r'\bedge\b|\boutline\b',p): return "Detecting edges!\n<OP>{\"intent\":\"edge\",\"params\":{}}</OP>"
    if re.search(r'\bface.?detect\b|\bdetect.?face\b|\bfind.?face\b',p): return "Detecting faces!\n<OP>{\"intent\":\"face_detect\",\"params\":{}}</OP>"
    if re.search(r'\bcolor.?palette\b|\bextract.?color\b|\bpalette\b',p): return "Extracting color palette!\n<OP>{\"intent\":\"color_palette\",\"params\":{}}</OP>"
    if re.search(r'\bcolor.?analys\b|\banalyz.?color\b',p): return "Analyzing colors!\n<OP>{\"intent\":\"color_analysis\",\"params\":{}}</OP>"
    if re.search(r'\bquality\b|\bcheck.?image\b',p): return "Checking image quality!\n<OP>{\"intent\":\"quality_check\",\"params\":{}}</OP>"
    if re.search(r'\bhistogram\b',p): return "Equalizing histogram!\n<OP>{\"intent\":\"histogram_eq\",\"params\":{}}</OP>"
    if re.search(r'\bsuper.?resol\b|\bupscale\b|\bupscal\b|\benlarge\b',p): return "Applying super resolution!\n<OP>{\"intent\":\"super_resolution\",\"params\":{}}</OP>"
    if re.search(r'\bdeblur\b|\bun.?blur\b|\bfix.?blur\b',p): return "Deblurring!\n<OP>{\"intent\":\"deblur\",\"params\":{}}</OP>"
    if re.search(r'\bcoloriz\b|\badd.?color\b',p): return "Colorizing image!\n<OP>{\"intent\":\"colorize_bw\",\"params\":{}}</OP>"
    if re.search(r'\bscratch\b|\bold.?photo\b|\brestore\b',p): return "Restoring image!\n<OP>{\"intent\":\"restore_old\",\"params\":{}}</OP>"
    if re.search(r'\bpixelat\b',p):
        m=re.search(r'(\d+)',p); sz=int(m.group(1)) if m else 10
        return f"Pixelating!\n<OP>{{\"intent\":\"pixelate\",\"params\":{{\"size\":{sz}}}}}</OP>"
    if re.search(r'\bnoise\b|\bgrain\b|\bfilm.?grain\b',p): return "Adding film grain!\n<OP>{\"intent\":\"noise\",\"params\":{}}</OP>"
    if re.search(r'\bvignet\b',p): return "Applying vignette!\n<OP>{\"intent\":\"vignette\",\"params\":{}}</OP>"
...
def process_image(img, intent, params):
    ...
    if intent=='crop':
        box=params.get('box')
        if not box:
            w,h=img.size; pad_w,pad_h=w//8,h//8
    ...
    if intent=='sharpen': return img.filter(ImageFilter.UnsharpMask(radius=2,percent=200,threshold=3))
    if intent=='edge':
    ...
    if intent=='glitch':
        ...
        for _ in range(20):
            y=np.random.randint(0,h); h2=np.random.randint(3,20)
    ...
    if intent=='pop_art':
        ...
        for i,c in enumerate(colors):
            ...
            result=Image.composite(col_fg,col_bg,Image.fromarray(thresh))
    ...
    if intent=='stained_glass':
        ...
        _,labels,centers=cv2.kmeans(Z,12,None,criteria,10,cv2.KMEANS_RANDOM_CENTERS)
    ...
    if intent=='pointillism':
        ...
        ys=np.random.randint(0,h2,size=n_dots); xs=np.random.randint(0,w2,size=n_dots)
    ...
    if intent=='ascii_art':
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                avg=patch.mean(); idx=int(avg/255*(len(chars)-1))
                draw.text((c*cell,r*cell),chars[idx],fill=(int(avg),int(avg),int(avg)))
        return canvas
    if intent=='thermal_vision':
        gray=np.array(ImageOps.grayscale(img))
        return cv2_to_pil(cv2.applyColorMap(gray,cv2.COLORMAP_JET))
    ...
    if intent=='tilt_shift':
        ...
        for y in range(h2):
            ...
            elif dist<zone*3: mask[y]=max(0,1.0-(dist-zone)/(zone*2))
    ...
    if intent=='bokeh':
        ...
        mask=cv2.GaussianBlur(mask.astype(np.float32),(61,61),0)[:,:,np.newaxis]
    ...
    if intent=='fisheye':
        ...
        return cv2_to_pil(cv2.undistort(cv_img,K,D))
    if intent=='mosaic':
        arr=np.array(img); h2,w2=arr.shape[:2]
        ...
        for y in range(0,h2,block):
            for x in range(0,w2,block):
                ...
                arr[y:y+block,x:x+block]=patch.mean(axis=(0,1)).astype(np.uint8)
        return Image.fromarray(arr)
    if intent=='pixelate':
    ...
    if intent=='vignette':
        ...
        mask=ky*kx.T; mask=mask/mask.max(); mask=mask**0.5
    ...
    if intent=='skin_analyze':
        ...
        return cv2_to_pil(cv2.detailEnhance(enhanced,sigma_s=10,sigma_r=0.15))
    ...
    if intent=='segment':
        ...
        cleaned=cv2.morphologyEx(thresh,cv2.MORPH_CLOSE,np.ones((3,3),np.uint8))
    ...
    if intent=='canny':
        gray=np.array(ImageOps.grayscale(img))
        return Image.fromarray(cv2.Canny(gray,50,150)).convert("RGB")
    ...
    if intent=='color_palette':
        ...
        return Image.fromarray(np.vstack([img_arr,palette]).astype(np.uint8))
    ...
    if intent=='quality_check':
        ...
        noise=float(gray.std()); brightness=float(gray.mean())
        ...
        texts=[f"Sharpness: {quality} ({blur_score:.0f})",f"Brightness: {brightness:.0f}/255",
               f"Resolution: {img.width}x{img.height}",f"Noise level: {noise:.1f}"]
    ...
    if intent=='restore_old':
        ...
        return cv2_to_pil(result).filter(ImageFilter.UnsharpMask(radius=1,percent=100,threshold=3))
...
def api_validate_email():
    data=request.json or {}
    return jsonify({"valid":validate_email(data.get("email",""))})

@app.route("/api/auth/validate-username", methods=["POST"])
def api_validate_username():
    data=request.json or {}
    return jsonify({"valid":validate_username(data.get("username",""))})
...
def share_chat():
    try:
        ...
        chat_title = data.get("title", "Shared Chat")
...
def view_shared_chat(share_id):
    chat = shared_chats.get(share_id)
    if not chat:
        return "<h2>Chat not found or expired.</h2>", 404
    return f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>{chat['title']} — Lumina</title>
<link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
<style>
body{{background:linear-gradient(135deg,#6a7bd1,#41e1ae);min-height:100vh;font-family:Segoe UI,sans-serif;padding:20px}}
.container{{max-width:760px;margin:0 auto;background:rgba(0,0,0,0.3);border-radius:20px;padding:24px;backdrop-filter:blur(12px)}}
...
.user-msg{{background:rgba(255,255,255,0.88);color:#1a1a2e;margin-left:auto}}
.bot-msg{{background:rgba(0,0,0,0.38);color:white}}
.back-btn{{display:inline-block;margin-top:16px;padding:8px 18px;background:white;color:#6a7bd1;border-radius:20px;text-decoration:none;font-weight:600}}
</style></head><body>
<div class="container">
<h2>📤 {chat['title']}</h2>
<p style="color:rgba(255,255,255,0.6);font-size:12px;margin-bottom:16px">Shared on {chat['created'][:10]}</p>
{chat['html']}
<br><a href="/" class="back-btn">← Open Lumina</a>
</div></body></html>"""
...
def api_status():
    status = {"gemini": bool(gemini_client), "claude": bool(claude_client) if ENABLE_CLAUDE else False,
              "local": True, "mode": "gemini-primary"}
    if not gemini_client:
        status["warning"] = "⚠️ No GEMINI_API_KEY set."
...
def test_ai():
    ...
    if gemini_client:
        try:
            response = gemini_client.models.generate_content(model=GEMINI_MODEL, contents="Say hello in one word")
