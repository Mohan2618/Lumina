...
# ─────────────────────────────────────────────────────────────
#  GEMINI SETUP  (free tier — no credit card needed)
# ─────────────────────────────────────────────────────────────
...
if GEMINI_API_KEY:
    ...
    gemini_model = genai.GenerativeModel(
        ...
        system_instruction="""You are Lumina, a friendly and expert AI image processing assistant.
...
When the user asks you to PERFORM an image operation, reply with a friendly explanation AND include this exact tag at the very end of your message:
...
- No image uploaded but operation requested → ask them to upload one
...
def call_gemini(history: list, user_text: str, image_pil=None) -> str:
    ...
    parts.append(user_text if user_text else "Please describe and analyze this image in detail.")
...
# ─────────────────────────────────────────────────────────────
#  FREE FALLBACK  (when no API key — rule-based)
# ─────────────────────────────────────────────────────────────
...
def analyze_image_free(img):
    ...
    bright = "very bright" if brightness>200 else "well-lit" if brightness>140 else "moderately lit" if brightness>80 else "dark/low-light"
...
def free_reply(prompt, has_image, img=None):
    ...
    if has_image and re.search(r'what|describe|tell|analyz|explain|identify|see|show|caption|about|who|where', p):
        ...
        return (f"**Image Analysis:**\n\n"
                ...
                f"*Add your GEMINI_API_KEY for full AI-powered image understanding.*")
    ...
    replies = {
        ...
        r'what can you do|help|feature': (
            ...
            "**Describe:** What's in this image? What's the mood?\n"
    ...
    return ("I'm not sure what you'd like. Try:\n"
            "• Upload an image and ask *'describe this'*\n"
            "• *'Make it grayscale'*, *'increase contrast'*, *'rotate 90 degrees'*\n"
            "• *'What can you do?'*")
...
def detect_op(p):
    ...
    if re.search(r'\bsaturat\b|\bvibran\b|\bcolor.boost\b', p):
    ...
    if re.search(r'\bpixelat\b|\bpixel.?art\b', p):
    ...
    if re.search(r'\bsegment\b|\bothsu\b|\bthreshold\b|\bbinary\b', p):
...
def process_image(img, intent, params):
    ...
    if intent == 'info':
        ...
        return None   # info returns text only — handled in route
...
def process():
    try:
        ...
        if intent == 'info' and image_pil:
            ...
            mr,mg,mb = arr[:,:,0].mean(),arr[:,:,1].mean(),arr[:,:,2].mean()
            clean_reply += f"\n\n**Image Info:** {w}×{h}px | Avg RGB: ({mr:.0f},{mg:.0f},{mb:.0f}) | {w*h:,} total pixels"
        ...
        if "API_KEY_INVALID" in err or "API key not valid" in err:
            msg = "⚠️ Invalid Gemini API key. Please check your GEMINI_API_KEY secret in Hugging Face Settings."
        elif "quota" in err.lower() or "429" in err:
            msg = "⚠️ Free API limit reached. Please wait a minute and try again (free tier: 15 requests/min)."
        else:
            msg = f"⚠️ Error: {err}"
