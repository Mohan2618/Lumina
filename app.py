...
# ─────────────────────────────────────────────────────────────
#  SYSTEM PROMPT — Fixed identity, no Google references
# ─────────────────────────────────────────────────────────────
SYSTEM_PROMPT = """You are Lumina, an advanced AI image processing assistant and general-purpose AI created by a team of 4 developers: Mohan Lingabathina, Hevendra Bage, Sowrya, and Karthikeya. You are NOT made by Google, NOT made by Anthropic, NOT made by OpenAI. You are Lumina, built by this team.

IDENTITY RULES (VERY IMPORTANT):
- If asked "who created you" or "who made you": say "I was created by a team of 4 developers: Mohan Lingabathina, Hevendra Bage, Sowrya, and Karthikeya."
- If asked "what are you" or "who are you": say "I am Lumina, an AI image processing assistant."
- NEVER say you are trained by Google, Anthropic, OpenAI, or any other company.
- NEVER say you are Gemini, Claude, ChatGPT, or any other AI.
- You use AI technology under the hood, but you are Lumina.

You can:
1. Describe and analyze images in rich detail
2. Perform medical image analysis with proper disclaimers
...
- When user asks to perform ANY image operation (blur, sharpen, rotate, filter, etc.), DO IT IMMEDIATELY.
- Do NOT ask for clarification on which type of blur/filter/effect — just apply the operation with good defaults.
- Do NOT ask the user to choose between options — just pick the best default and apply it.
- WRONG behavior: "Which kind of blur? Gaussian, Motion, or Surface?" — NEVER do this.
- CORRECT behavior: "Applying blur to your image!" then the <OP> tag.
- If user says "blur image" → apply gaussian blur immediately.
- If user says "make it artistic" → apply watercolor or oil painting.
- Always be decisive and action-oriented.

OPERATION TAG FORMAT:
When the user asks to PERFORM an image operation, reply with a SHORT confirmation AND include this exact tag at the very end:
...
- "blur image" → <OP>{"intent":"blur","params":{"radius":5}}</OP>
- "make grayscale" → <OP>{"intent":"grayscale","params":{}}</OP>
- "enhance this X-ray" → <OP>{"intent":"xray_enhance","params":{}}</OP>
- "surface blur" → <OP>{"intent":"blur","params":{"radius":8}}</OP>
- "gaussian blur" → <OP>{"intent":"blur","params":{"radius":5}}</OP>
- "motion blur" → <OP>{"intent":"blur","params":{"radius":10}}</OP>
- "generate a sunset" → <OP>{"intent":"generate_image","params":{"prompt":"a beautiful sunset over the ocean with golden sky, photorealistic, 4K"}}</OP>

Rules:
- For descriptions/questions: reply naturally, NO <OP> tag
- For operations: short confirmation + <OP> tag immediately, no asking for choices
- NEVER ask the user what type of blur, filter, or effect — just apply a sensible default
- Be warm, concise, action-oriented
...
def call_claude(history: list, user_text: str, image_pil=None) -> str:
    ...
    response = claude_client.messages.create(
        ...
        system=SYSTEM_PROMPT, messages=messages, timeout=20
    )
...
def call_ai(history: list, user_text: str, image_pil=None) -> tuple:
    ...
    if not reply:
        reply = "✨ I'm Lumina! Upload an image to process it, or ask me anything!"
    return reply, "local"
...
def generate_image_from_prompt(prompt: str):
    print(f"[Flux] Generating: {prompt[:100]}...")
    ...
    try:
        ...
        return create_flux_placeholder(prompt, "Generation failed. Try again in a moment.")
...
def free_reply(prompt, has_image, img=None):
    ...
    # Identity questions — fixed
    if re.search(r'who (created|made|built|developed) you|your (creator|maker|developer)', p):
        return "I was created by a team of 4 developers: **Mohan Lingabathina**, **Hevendra Bage**, **Sowrya**, and **Karthikeya**. I am Lumina, their AI image processing assistant! 🌟"
    if re.search(r'who are you|what are you|your name', p):
        return "I'm **Lumina** — an AI image processing assistant built by a team of 4 developers. I can analyze images, apply filters, do medical imaging, generate art, and much more!"
    ...
    replies = {
        r'hello|hi\b|hey\b': "Hi there! 👋 I'm **Lumina**, your AI image assistant. Upload an image or ask me anything!",
        r'thank': "You're welcome! 😊",
        r'bye|goodbye': "Goodbye! Come back anytime! 👋",
        r'how are you': "I'm doing great! Ready to help with your images. What would you like to do?",
    }
    for pat,rep in replies.items():
        if re.search(pat,p): return rep

    if p:
        return (f"I understand you're asking: *\"{prompt[:80]}{'...' if len(prompt)>80 else ''}\"*\n\n"
                "I'm **Lumina**, your AI image assistant. Here's what I can do:\n"
                "• 🖼️ Upload an image to analyze or process\n"
                "• 🤖 Generate images from text\n"
                "• 🏥 Medical image analysis\n"
                "• 🎨 100+ creative filters and effects")
    return "Upload an image and ask me to describe, filter, or process it — or generate a new image from a text prompt!"


def detect_op(p):
    if not p: return None

    if re.search(r'\bgenerat\w*\b|\bcreate\s+(a|an|image|picture|photo|art)\b|\bdraw\b|\bpaint\b', p):
        prompt_match = re.sub(r'^(generate|create|make|draw|paint)\s+(a|an|the|image|picture|photo|of|art)?\s*', '', p, flags=re.I).strip()
        if not prompt_match or len(prompt_match) < 3: prompt_match = p
        return f"✨ Generating your image...\n<OP>{{\"intent\":\"generate_image\",\"params\":{{\"prompt\":\"{prompt_match}, highly detailed, 4K, professional\"}}}}</OP>"

    # Blur — handle all types immediately, no questions
    if re.search(r'\bsurface\s+blur\b', p):
        return "Applying surface blur!\n<OP>{\"intent\":\"blur\",\"params\":{\"radius\":8}}</OP>"
    if re.search(r'\bmotion\s+blur\b', p):
        return "Applying motion blur!\n<OP>{\"intent\":\"blur\",\"params\":{\"radius\":10}}</OP>"
    if re.search(r'\bgaussian\s+blur\b', p):
        return "Applying Gaussian blur!\n<OP>{\"intent\":\"blur\",\"params\":{\"radius\":5}}</OP>"
    if re.search(r'\bblur\b|\bsmooth\b|\bsoft\b|\bsoften\b', p):
        m=re.search(r'radius\D*(\d+)',p); r=int(m.group(1)) if m else 5
        return f"Blurring your image!\n<OP>{{\"intent\":\"blur\",\"params\":{{\"radius\":{r}}}}}</OP>"
    ...
    if re.search(r'\bvintage\b|\blomo\b',p):
    ...
    if re.search(r'\boil paint\b|\boil art\b|\boil painting\b',p):
...
def process_image(img, intent, params):
    ...
    if intent=='blur':
        radius=max(1,params.get('radius',5))
    ...
    if intent=='pop_art':
        ...
        for i,c in enumerate(colors):
            ...
            gray_s=ImageOps.grayscale(small); arr_g=np.array(gray_s)
    ...
    if intent=='stained_glass':
        ...
        K=12; criteria=(cv2.TERM_CRITERIA_EPS+cv2.TERM_CRITERIA_MAX_ITER,20,1.0)
    ...
    if intent=='ascii_art':
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                char=chars[idx]; brightness=int(avg)
    ...
    if intent=='morphology':
        op=params.get('op','dilate'); gray=np.array(ImageOps.grayscale(img))
    ...
    if intent=='color_palette':
        ...
        K=8; criteria=(cv2.TERM_CRITERIA_EPS+cv2.TERM_CRITERIA_MAX_ITER,20,1.0)
        _,labels,centers=cv2.kmeans(Z,K,None,criteria,10,cv2.KMEANS_RANDOM_CENTERS)
    ...
    if intent=='quality_check':
        ...
        texts=[f"Sharpness: {quality} ({blur_score:.0f})",f"Brightness: {brightness:.0f}/255",
               f"Resolution: {img.width}x{img.height}",f"Noise: {noise:.1f}"]
...
def api_status():
    ...
    if not gemini_client:
        status["warning"]="⚠️ No GEMINI_API_KEY. Get one free: https://aistudio.google.com/app/apikey"
...
def process():
    try:
        ...
        if file and file.filename and image_pil:
            new_user_parts.append({"mime_type":"image/jpeg",
        ...
        elif is_rate_limit(err): msg="⚠️ API rate limit hit. Please try again shortly."
        elif "not found" in err.lower() or "404" in err: msg=f"⚠️ Model not found: {err}"
        else: msg=f"⚠️ Error: {err}"
